...
const ScrollStack: React.FC<ScrollStackProps> = ({
  ...
  const getDocumentOffset = useCallback((element: HTMLElement) => {
    let offset = 0;
  ...
  const recomputeOffsets = useCallback(() => {
    ...
    if (useWindowScroll) {
      cardOffsetsRef.current = cardsRef.current.map(getDocumentOffset);
      endElementOffsetRef.current = endElementRef.current
        ? getDocumentOffset(endElementRef.current)
        : 0;
    } else {
      cardOffsetsRef.current = cardsRef.current.map((card) => card.offsetTop);
      endElementOffsetRef.current = endElementRef.current
        ? endElementRef.current.offsetTop
        : 0;
    }
